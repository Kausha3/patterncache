...
export const rateLimiter: SDLesson = {
  ...
  blurb: "Reject the excess fast, and keep the count consistent across every server.",
  estMinutes: 6,
  overview:
    "A rate limiter caps how many requests a caller can make in a window. That's what protects your system from abuse and overload. The interesting parts are the algorithm (how you count) and the distribution problem (many servers have to agree on one caller's count). Each stage tightens both. Click any node to inspect it.",
  stages: [
    {
      ...
      tradeoff:
        "Zero latency, but it's wrong the moment you run more than one server: a caller spread across N servers gets N× their limit. Fixed windows also allow a burst at the boundary (2× at the window edge).",
    ...
    },
    {
      ...
      problem:
        "With multiple app servers, the limit has to be global. Every server needs to see the same count for a given caller.",
      ...
      tradeoff:
        "Correct across the fleet, but you've added a network hop to every request and made that store a dependency on the hot path, so it needs to be highly available and low latency.",
    ...
    },
    {
      ...
      fix: "Use a token-bucket algorithm (tokens refill at a steady rate and each request spends one, so it's smooth and allows controlled bursts) and make the check-and-decrement a single atomic operation in the store so concurrent requests can't both slip through.",
      tradeoff:
    ...
    },
    {
      ...
      problem:
        "Even a fast rejection still travels deep into your system before being turned away, so abusive traffic ends up eating load balancer and app capacity just to get denied.",
      ...
      tradeoff:
        "Abusive load is shed cheaply and far from your app, at the cost of keeping edge counters roughly in sync across regions. Usually eventual consistency is fine for limiting, so you trade a little exactness for a lot of protection.",
  ...
  recap: [
    ...
    "One server's in-memory counter is wrong the moment you scale out, so the limit has to live in a shared store.",
    ...
    "Push enforcement toward the edge so excess traffic gets rejected before it costs you. You accept eventual consistency on the counters as the tradeoff.",
  ],
  ...
  interview: {
    ...
    questions: [
      ...
      {
        id: "scale",
        ask: "How many callers and requests/sec are we protecting? Just one service, or a large fleet of servers?",
        category: "scale",
        answer: "Millions of requests/sec across a large fleet of servers, globally.",
        why: "Many servers is the whole challenge here. An in-memory counter on one box is wrong the moment traffic spreads, so scale forces a shared counter.",
      ...
      },
      {
        ...
        branches: [
          ...
          { label: "Strictly exact", approach: "Every check-and-decrement must be atomic on one source of truth. Correct, but that's a hot-path round-trip and a scaling bottleneck." },
        ],
      },
      {
        id: "bursts",
        ...
        category: "constraints",
        answer: "Allow small controlled bursts. A caller shouldn't get blocked for a brief, reasonable spike.",
      ...
      },
      {
        ...
        category: "premature",
        redirect: "Deployment shape comes after the algorithm and counter store, so let's hold it until we know the accuracy and scale needs.",
      },
    ],
  },
};

...
export const cacheLayer: SDLesson = {
  ...
  stages: [
    ...
    },
    {
      ...
      visibleNodes: ["client", "app", "cache", "origin"],
      problem:
        "After an update, the cache still holds the old value, so readers see stale data until something clears it.",
      ...
      tradeoff:
        "Freshness improves, but every policy has a cost. Write-through slows writes and can cache data nobody reads, and TTLs are a blunt instrument: too short kills hit-rate, too long serves stale data. There's no free correctness.",
    ...
    },
    {
      ...
      problem:
        "Memory is finite, so the cache evicts. When a hot key expires, thousands of simultaneous misses can stampede the origin all at once (a 'cache stampede').",
      ...
      tradeoff:
        "This is stable under memory pressure and immune to stampedes, but eviction tuning is workload-specific and stampede protection adds coordination. You end up accepting slightly staler reads during a rebuild to protect the origin.",
    ...
    },
    {
      ...
      tradeoff:
        "This scales in both capacity and hot-key throughput, at the cost of a rebalancing story (consistent hashing so adding a node doesn't cold-miss everything) and replicating the hottest keys, which is just memory spent to defeat a hotspot.",

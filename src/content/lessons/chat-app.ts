...
export const chatApp: SDLesson = {
  ...
  stages: [
    ...
    },
    {
      ...
      tradeoff:
        "Decoupled and horizontally scalable, but async fan-out makes ordering a real concern. Recipients have to see a conversation's messages in the same order, so you now enforce per-conversation sequencing, not global.",
  ...
  recap: [
    ...
    "Async delivery buys scale but costs you easy ordering, so you enforce per-conversation sequencing to get it back.",
  ],
    ...
    questions: [
      {
        ...
        branches: [
          ...
          { label: "Huge broadcast rooms (100k+)", approach: "Fan-out-on-write explodes here, so switch large rooms to fan-out-on-read (members pull), treating it more like a feed." },
        ],
      },
      {
        id: "scale",
      ...
      },
      {
        ...
        answer: "Messages must not be lost (at-least-once delivery) and must appear in the same order within a conversation.",
        ...
        branches: [
          ...
          { label: "Exactly-once, global order", approach: "Much harder and rarely needed. Global ordering across all conversations kills throughput, so push back on this ask." },
        ],
      },
      {
        id: "offline",
        ...
        category: "constraints",
        answer: "Yes, offline users get a push notification and see the message when they come back. Keep full history.",
      ...
      },
      {
        ...
        category: "premature",
        redirect: "Let's settle transport once we know the delivery and scale needs, so hold that thought. (It'll be persistent connections, but establish why first.)",
      },
      {
        id: "db-premature",

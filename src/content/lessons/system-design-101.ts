...
export const systemDesign101: SDLesson = {
  ...
  stages: [
    ...
    },
    {
      ...
      fix: "In a database: durable storage that remembers data between requests, even if a server crashes and restarts. Think of it as the system's long-term memory. The server is the worker, and the database is what it reads from and writes to. Keeping the server itself 'stateless' (holding no important data of its own) is the quiet trick that lets you run many servers later.",
      tradeoff: "Splitting compute (the server) from storage (the database) is the first move in almost every design. Say it out loud, 'stateless app servers, a database for persistence,' and it instantly signals you know the shape.",
    ...
    },
    {
      ...
      fix: "Latency is how long one request takes in milliseconds, the wait a user feels. Throughput is how many requests per second the system can handle before it chokes, basically its capacity. Drag the load slider below: with headroom, latency stays flat and nothing drops. Push past the throughput ceiling and latency spikes while requests get shed. Every tradeoff in every later lesson is really about moving these two numbers.",
    ...
    },
    {
      ...
      visibleNodes: ["client", "app", "db"],
      problem: "One server was fine for a handful of users. Drag the load up and see what breaks when a million show up at once.",
      fix: "Every single machine has a ceiling: CPU, memory, open connections. Past it, latency spikes and requests start failing (you just watched it happen). And you can't buy a bigger machine forever. So the entire rest of system design is answering one question, in different ways for different problems: how do we serve more load than a single server can? More servers behind a load balancer, caches, queues, replicas: each lesson answers it a different way.",

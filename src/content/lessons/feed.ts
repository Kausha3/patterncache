...
export const feed: SDLesson = {
  ...
  stages: [
    {
      ...
      problem:
        "The simplest feed: when a user opens the app, query the posts of everyone they follow, merge, sort by time, return. Correct, but every single feed load runs a big fan-in query, and popular users get read constantly.",
    ...
    },
    {
      ...
      problem:
        "Read-time merging can't keep up at scale. Assembling a feed from hundreds of followees on every open is just too much work per read.",
    ...
    },
    {
      ...
      tradeoff:
        "You get the best of both: cheap reads for the common case and no write explosion for celebrities. That costs you two code paths and a threshold to tune (when does an account become 'celebrity'?). That threshold is the real design lever.",
  ...
  recap: [
    ...
    "Fan-out-on-write makes reads O(1) by paying at write time, and that's worth it only because reads dwarf writes.",
  ...
  interview: {
    ...
    summary:
      "Clarified: a read-heavy, reverse-chronological feed for hundreds of millions of users, with celebrity accounts in the mix, and a few seconds of delay for new posts is fine. That points straight at fan-out-on-write for the common case (precomputed timelines), a hybrid merge-on-read path for celebrities, and heavy caching. That's exactly the design we build next.",
    questions: [
      {
        id: "scope",
        ...
        why: "Ranked vs chronological changes the read path a lot. Ranking adds a scoring step and usually more precomputation, so pin it down early.",
        ...
        branches: [
          ...
          { label: "Ranked / ML", approach: "Add a ranking service that scores candidate posts per user. That's more compute, a candidate-generation step, and usually precomputed." },
        ],
      },
      {
        id: "graph",
        ...
        lp: ["dive-deep"],
        branches: [
          { label: "Everyone ~hundreds of followers", approach: "Fan-out-on-write works cleanly here: precompute each follower's timeline the moment someone posts." },
      ...
      {
        id: "ratio",
        ask: "How does viewing compare to posting? Is it read-heavy or write-heavy?",
        category: "scale",
      ...
      },
      {
        ...
        category: "premature",
        redirect: "Cache tech is a later detail. First decide fan-out-on-write vs read, since that determines what you're even caching.",
      },
    ],
  },
};

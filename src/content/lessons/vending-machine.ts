  ...
  design: {
    entities: [
      {
        ...
        name: "VendingMachine",
        isEntity: true,
        why: "The top-level system — holds the current state and coordinates a purchase end to end.",
        properties: [
          { name: "id", type: "string" },
      ...
      },
      {
        ...
        name: "Slot",
        isEntity: true,
        why: "A single row — tracks its own product, price, and remaining quantity.",
        properties: [
          { name: "id", type: "string" },
      ...
      },
      {
        ...
        name: "Product",
        isEntity: true,
        why: "An item for sale — has a price and a name, independent of which slot holds it.",
        properties: [
          { name: "id", type: "string" },
    ...
    methods: [
      ...
      },
      {
        ...
        ownerId: "machine",
        justification: "Cancelling touches both Payment (issue a refund) and VendingMachineState (transition back to idle) — it has to live on the one class holding references to both.",
      },
      {
        id: "m4",
        signature: "getProduct(): Product",
        ownerId: "slot",
        justification: "Slot is the only class holding a reference to which Product it's stocked with — a plain accessor for data it already owns.",
      },
      {
        id: "m5",
      ...
      },
      {
        ...
        ownerId: "product",
        justification: "Price is data Product holds directly — a plain accessor, not a decision, so it belongs on the object whose field it's reading.",
      },
      {
        id: "m8",
      ...
      },
      {
        ...
        codeExercise: {
          ...
          checklist: [
            ...
            "Doesn't mutate amountInserted as a side effect — computing change is separate from resetting the payment",
    ...
    principles: [
      ...
      {
        name: "State pattern",
        explanation: "VendingMachineState is a real class, not just an enum field on VendingMachine, specifically so canSelect() and transitionTo() can hold the actual gating logic — the concrete reason bad sequences become impossible by construction instead of just handled with an if-check.",
      },
      {

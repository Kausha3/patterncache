...
export const slidingWindow: DSALesson = {
  ...
  concept: {
    bruteForce:
      "The obvious move is to check every possible substring: start at each index, extend until you hit a repeat, and track the longest. It works, but you re-scan the same characters again and again.",
    ...
    complexity: "O(n)",
    recognize:
      "Reach for a sliding window when a problem asks for the best (longest / shortest / max-sum) contiguous run that satisfies some constraint, and a brute force would end up re-scanning overlapping ranges.",
  },
  trace: {
    input: "abcabcbb",

  ...
  stages: [
    {
      ...
      tradeoff: "Simplest possible thing. Zero redundancy, zero headroom, and a single point of failure.",
    ...
    },
    {
      ...
      problem:
        "Redirects hammer the database, but traffic is heavily skewed. A tiny fraction of links get most of the clicks, so the DB ends up doing repetitive work for popular codes.",
      ...
      tradeoff:
        "Huge read speedup, but you've added a consistency question: if a mapping changes, stale cache entries must be invalidated. For a shortener, mappings are effectively immutable, so this is cheap and a great fit.",
    ...
    },
    {
      ...
      tradeoff:
        "Big horizontal headroom. Replicas do introduce replication lag (a just-created link might briefly miss on a replica), but that's acceptable for a shortener where creates are rare and a retry resolves it.",
    ...
    },
    {
      ...
      tradeoff:
        "Near-user latency and collision-free code generation, at the cost of more moving parts to operate. The KGS becomes a component you must keep highly available, but in exchange it removes a whole class of write bugs.",
